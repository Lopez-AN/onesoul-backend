...
-- Server version:               10.6.24-MariaDB-log - managed by https://aws.amazon.com/rds/
-- Server OS:                    Linux
...
CREATE TABLE IF NOT EXISTS `Bookings` (
  ...
  `LastBookingEvent` enum('Pending','Rescheduled','Modified','Canceled','GuideRated','SeekerRated','Completed') DEFAULT NULL,
  ...
  KEY `idx_bookings_offering_event` (`OfferingID`,`LastBookingEvent`),
...
CREATE TABLE IF NOT EXISTS `BookingStatus` (
  ...
  `BookingEvent` enum('Pending','Rescheduled','Modified','Canceled','GuideRated','SeekerRated','Completed') DEFAULT NULL COMMENT 'Indica el estado del booking (creación, reprogramación, cancelación, calificación, etc.)',
...
CREATE TABLE IF NOT EXISTS `Countries` (
  `CountryCode` char(2) NOT NULL COMMENT 'Código ISO 3166-1 alpha-2',
  `CurrencyCode` char(3) NOT NULL COMMENT 'Moneda asociada al país',
  `CountryName` varchar(50) NOT NULL COMMENT 'Nombre completo del país',
  `PhoneCode` varchar(5) NOT NULL COMMENT 'Prefijo telefónico',
  `IsActive` tinyint(1) NOT NULL DEFAULT 1 COMMENT 'Indica si el país está activo.',
  PRIMARY KEY (`CountryCode`),
  KEY `FK_Countries_Currencies` (`CurrencyCode`),
  CONSTRAINT `FK_Countries_Currencies` FOREIGN KEY (`CurrencyCode`) REFERENCES `Currencies` (`CurrencyCode`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Almacena información sobre los países.';

-- Data exporting was unselected.

-- Dumping structure for table soul.CountriesStates
CREATE TABLE IF NOT EXISTS `CountriesStates` (
  `CountryCode` char(2) NOT NULL COMMENT 'Código pais ISO 3166-1 alpha-2',
  `StateName` varchar(75) NOT NULL COMMENT 'Nombre del estado o provincia',
  PRIMARY KEY (`CountryCode`,`StateName`),
  CONSTRAINT `FK_CountriesStates_Countries` FOREIGN KEY (`CountryCode`) REFERENCES `Countries` (`CountryCode`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Guarda los estados/provincias (o como se denomine en dicho pais sus unidades administrativas) ';
...
-- Dumping structure for table soul.CurrencyExchangeRates
CREATE TABLE IF NOT EXISTS `CurrencyExchangeRates` (
  `Date` date NOT NULL DEFAULT curdate() COMMENT 'Fecha cotización',
  `BaseCurrency` char(3) NOT NULL COMMENT 'Moneda base',
  `QuoteCurrency` char(3) NOT NULL COMMENT 'Moneda a convertir',
  `ExchangeRate` float unsigned NOT NULL COMMENT 'Tasa de cambio',
  PRIMARY KEY (`Date`,`BaseCurrency`,`QuoteCurrency`) USING BTREE,
  KEY `FK_CurrencyExchangeRates_Currencies1` (`QuoteCurrency`) USING BTREE,
  KEY `FK_CurrencyExchangeRates_Currencies2` (`BaseCurrency`) USING BTREE,
  CONSTRAINT `FK_CurrencyExchangeRates_Currencies1` FOREIGN KEY (`QuoteCurrency`) REFERENCES `Currencies` (`CurrencyCode`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_CurrencyExchangeRates_Currencies2` FOREIGN KEY (`BaseCurrency`) REFERENCES `Currencies` (`CurrencyCode`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Guarda las cotizaciones de las monedas entre si';
...
CREATE TABLE IF NOT EXISTS `DonationVouchers` (
  ...
  `Status` enum('Pending','Assigned','Redeemed','Canceled','Expired') NOT NULL DEFAULT 'Pending' COMMENT 'Estado de la donación',
...
-- Dumping structure for table soul.LandingContacts
CREATE TABLE IF NOT EXISTS `LandingContacts` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Date` datetime DEFAULT current_timestamp() COMMENT 'Fecha y hora del contacto',
  `Name` varchar(100) NOT NULL,
  `Email` varchar(100) NOT NULL,
  `SocialNetwork` varchar(100) DEFAULT NULL COMMENT 'Red social del guía',
  `CountryCode` char(2) DEFAULT NULL COMMENT 'Código del país del guia',
  `City` varchar(60) DEFAULT NULL COMMENT 'Ciudad o localidad',
  `Role` enum('Guide','Seeker') NOT NULL COMMENT 'Rol de la persona que contacta',
  `Specialization` varchar(50) DEFAULT NULL COMMENT 'Especialización del guía',
  `LookingFor` varchar(255) DEFAULT NULL COMMENT 'que necesita el buscador',
  `Browser` varchar(255) DEFAULT NULL COMMENT 'Nombre del navegador',
  `BrowserVersion` varchar(50) DEFAULT NULL COMMENT 'Versión del navegador',
  ...
  `IP` varchar(45) DEFAULT NULL COMMENT 'Dirección IP del dispositivo desde el cual se realizó el contacto',
  PRIMARY KEY (`ID`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Guardo los datos de quienes contactan con el formulario de la landing page';

-- Data exporting was unselected.

-- Dumping structure for table soul.LandingEmails
CREATE TABLE IF NOT EXISTS `LandingEmails` (
  `ID` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `Date` datetime DEFAULT current_timestamp() COMMENT 'Fecha y hora del contacto',
  `Email` varchar(100) NOT NULL,
  `Browser` varchar(255) DEFAULT NULL COMMENT 'Nombre del navegador',
...
CREATE TABLE IF NOT EXISTS `Media` (
  ...
  KEY `FK_OfferingID` (`OfferingID`),
  KEY `FK_ReviewID` (`ReviewID`),
  KEY `idx_media_offering_type` (`OfferingID`,`MediaType`),
...
CREATE TABLE IF NOT EXISTS `Offerings` (
  ...
  `Status` enum('Active','Inactive','Deleted','Draft') DEFAULT NULL COMMENT 'Estado del servicio',
  ...
  `ShortDescription` varchar(300) DEFAULT NULL COMMENT 'Descripción corta de la publicación',
  ...
  KEY `FK_CategoryID` (`CategoryID`),
  KEY `FK_UserID` (`UserID`),
  KEY `idx_offerings_status` (`Approved`,`Status`,`OfferingID`) USING BTREE,
...
CREATE TABLE IF NOT EXISTS `OfferingsLocations` (
  ...
  KEY `FK_OfferingsLocations_soul.Offerings` (`OfferingID`),
  CONSTRAINT `FK_OfferingsLocations_UsersLocations` FOREIGN KEY (`UserID`, `LocationID`) REFERENCES `UsersLocations` (`UserID`, `LocationID`),
  CONSTRAINT `FK_OfferingsLocations_soul.Offerings` FOREIGN KEY (`OfferingID`) REFERENCES `Offerings` (`OfferingID`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `Referrals` (
  ...
  `ReferredUserID` int(10) unsigned NOT NULL COMMENT 'Identificador del usuario referido; se llena cuando el registro se completa.',
  ...
  CONSTRAINT `FK_Referrals_Users` FOREIGN KEY (`ReferredUserID`) REFERENCES `Users` (`UserID`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS `Reviews` (
  ...
  KEY `FK_BookingID` (`BookingID`),
  KEY `FK_OfferingID` (`OfferingID`),
  KEY `idx_reviews_offering` (`OfferingID`),
  KEY `idx_reviews_seeker` (`SeekerID`),
...
-- Dumping structure for table soul.SubscriptionPaymentRejections
...
CREATE TABLE IF NOT EXISTS `Users` (
  ...
  `ValidatedPhone` tinyint(1) DEFAULT 0 COMMENT 'Celular validado.',
...
CREATE TABLE IF NOT EXISTS `UsersLocations` (
  ...
  KEY `FK_UsersLocations_Countries` (`CountryCode`),
  CONSTRAINT `FK_UsersLocations_Countries` FOREIGN KEY (`CountryCode`) REFERENCES `Countries` (`CountryCode`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Dumping structure for trigger soul.trg_offerings_audit_delete
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
DELIMITER //
CREATE TRIGGER trg_offerings_audit_delete AFTER DELETE ON soul.Offerings FOR EACH ROW BEGIN   INSERT INTO soul.AuditLogs     (TableName, RecordID, Action, ChangedBy, ChangeDate, OldValue, NewValue)   VALUES (    'Offerings',     OLD.OfferingID,     'DELETE',     CURRENT_USER(),     NOW(),     JSON_OBJECT('OfferingID', OLD.OfferingID, 'Title', OLD.Title, 'Description', OLD.Description, 'CategoryID', OLD.CategoryID, 'UserID', OLD.UserID, 'Status', OLD.Status, 'CreationDate', OLD.CreationDate, 'ModificationDate', OLD.ModificationDate, 'Approved', OLD.Approved, 'ApprovalDate', OLD.ApprovalDate, 'Currency', OLD.Currency, 'Tags', OLD.Tags, 'SKU', OLD.SKU, 'Stock', OLD.Stock, 'ServiceType', OLD.ServiceType, 'ShortDescription', OLD.ShortDescription, 'Price', OLD.Price, 'SessionType', OLD.SessionType, 'Conditions', OLD.Conditions, 'Duration', OLD.Duration),    NULL    ); END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Dumping structure for trigger soul.trg_offerings_audit_insert
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
DELIMITER //
CREATE TRIGGER trg_offerings_audit_insert AFTER INSERT ON soul.Offerings FOR EACH ROW BEGIN   INSERT INTO soul.AuditLogs     (TableName, RecordID, Action, ChangedBy, ChangeDate, OldValue, NewValue)   VALUES (    'Offerings',     NEW.OfferingID,     'INSERT',     CURRENT_USER(),     NOW(),     NULL,      JSON_OBJECT('OfferingID', NEW.OfferingID, 'Title', NEW.Title, 'Description', NEW.Description, 'CategoryID', NEW.CategoryID, 'UserID', NEW.UserID, 'Status', NEW.Status, 'CreationDate', NEW.CreationDate, 'ModificationDate', NEW.ModificationDate, 'Approved', NEW.Approved, 'ApprovalDate', NEW.ApprovalDate, 'Currency', NEW.Currency, 'Tags', NEW.Tags, 'SKU', NEW.SKU, 'Stock', NEW.Stock, 'ServiceType', NEW.ServiceType, 'ShortDescription', NEW.ShortDescription, 'Price', NEW.Price, 'SessionType', NEW.SessionType, 'Conditions', NEW.Conditions, 'Duration', NEW.Duration)  ); END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;

-- Dumping structure for trigger soul.trg_offerings_audit_update
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO';
DELIMITER //
CREATE TRIGGER trg_offerings_audit_update AFTER UPDATE ON soul.Offerings FOR EACH ROW BEGIN   IF NOT (OLD.OfferingID <=> NEW.OfferingID AND OLD.Title <=> NEW.Title AND OLD.Description <=> NEW.Description AND OLD.CategoryID <=> NEW.CategoryID AND OLD.UserID <=> NEW.UserID AND OLD.Status <=> NEW.Status AND OLD.CreationDate <=> NEW.CreationDate AND OLD.ModificationDate <=> NEW.ModificationDate AND OLD.Approved <=> NEW.Approved AND OLD.ApprovalDate <=> NEW.ApprovalDate AND OLD.Currency <=> NEW.Currency AND OLD.Tags <=> NEW.Tags AND OLD.SKU <=> NEW.SKU AND OLD.Stock <=> NEW.Stock AND OLD.ServiceType <=> NEW.ServiceType AND OLD.ShortDescription <=> NEW.ShortDescription AND OLD.Price <=> NEW.Price AND OLD.SessionType <=> NEW.SessionType AND OLD.Conditions <=> NEW.Conditions AND OLD.Duration <=> NEW.Duration) THEN     INSERT INTO soul.AuditLogs       (TableName, RecordID, Action, ChangedBy, ChangeDate, OldValue, NewValue)     VALUES (      'Offerings',       OLD.OfferingID,       'UPDATE',       USER(),       NOW(),       JSON_OBJECT('OfferingID', OLD.OfferingID, 'Title', OLD.Title, 'Description', OLD.Description, 'CategoryID', OLD.CategoryID, 'UserID', OLD.UserID, 'Status', OLD.Status, 'CreationDate', OLD.CreationDate, 'ModificationDate', OLD.ModificationDate, 'Approved', OLD.Approved, 'ApprovalDate', OLD.ApprovalDate, 'Currency', OLD.Currency, 'Tags', OLD.Tags, 'SKU', OLD.SKU, 'Stock', OLD.Stock, 'ServiceType', OLD.ServiceType, 'ShortDescription', OLD.ShortDescription, 'Price', OLD.Price, 'SessionType', OLD.SessionType, 'Conditions', OLD.Conditions, 'Duration', OLD.Duration),       JSON_OBJECT('OfferingID', NEW.OfferingID, 'Title', NEW.Title, 'Description', NEW.Description, 'CategoryID', NEW.CategoryID, 'UserID', NEW.UserID, 'Status', NEW.Status, 'CreationDate', NEW.CreationDate, 'ModificationDate', NEW.ModificationDate, 'Approved', NEW.Approved, 'ApprovalDate', NEW.ApprovalDate, 'Currency', NEW.Currency, 'Tags', NEW.Tags, 'SKU', NEW.SKU, 'Stock', NEW.Stock, 'ServiceType', NEW.ServiceType, 'ShortDescription', NEW.ShortDescription, 'Price', NEW.Price, 'SessionType', NEW.SessionType, 'Conditions', NEW.Conditions, 'Duration', NEW.Duration)    );   END IF; END//
DELIMITER ;
SET SQL_MODE=@OLDTMP_SQL_MODE;
